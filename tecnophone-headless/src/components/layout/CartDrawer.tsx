    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          'lg:inset-y-0 lg:left-auto lg:right-0 lg:max-h-full lg:w-full lg:max-w-lg lg:rounded-l-3xl lg:rounded-tr-none lg:border-l lg:border-t-0',
          isOpen
      ...
      >
            ...
            <ul className="space-y-3">
              {items.map((item) => {
                ...
                <li
                  ...
                  className="flex gap-4 p-3 lg:p-4 bg-surface-50 rounded-2xl border border-surface-200 hover:border-surface-300 transition-all group"
                >
                  {/* Image */}
                  <Link
                    href={`/producto/${item.product.slug}`}
                    onClick={closeCart}
                    className="relative w-20 h-20 lg:w-24 lg:h-24 rounded-xl overflow-hidden bg-white border border-surface-200 flex-shrink-0 group-hover:border-primary-200 transition-colors"
                  >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    <Link
                      href={`/producto/${item.product.slug}`}
                      className="text-sm font-semibold text-gray-900 hover:text-primary-600 line-clamp-2 transition-colors leading-snug"
                      onClick={closeCart}
                    >
                      {item.product.name}
                    </Link>
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-surface-200 px-5 lg:px-6 py-5 space-y-3 bg-white">
            {/* Savings banner */}
            {totalSavings > 0 && (
              <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-3.5 py-2.5">
                <span className="text-emerald-700 text-xs font-bold">🎉 Estás ahorrando {formatPrice(totalSavings)}</span>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className={cn('font-semibold', remaining <= 0 ? 'text-emerald-600' : 'text-surface-600')}>
                  {remaining <= 0 ? 'Gratis' : 'Calculado al pagar'}
                </span>
              </div>
              <div className="h-px bg-surface-200" />
              <div className="flex justify-between items-center">
                <span className="font-bold text-gray-900">Total</span>
                <span className="text-xl lg:text-2xl font-extrabold text-gray-900 font-display">
                  {formatPrice(total)}
                </span>
              </div>
            </div>
            ...
            {/* Delivery estimate */}
            <div className="flex items-center gap-2 text-xs text-surface-600 bg-surface-50 rounded-lg px-3 py-2">
            ...
            <Link
              href="/checkout"
              onClick={closeCart}
              className="w-full flex items-center justify-center gap-2 bg-primary-600 text-white py-3.5 rounded-xl font-bold text-base shadow-lg shadow-primary-600/25 hover:bg-primary-700 hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 transition-all"
            >
            ...
            {/* Trust signals */}
            <div className="flex items-center justify-center gap-4 text-[11px] text-surface-500 pt-1">
